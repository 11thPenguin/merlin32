...
int BuildAllDataLineSize(struct omf_segment* current_omfsegment) {
    // JASNOTE: Why is this function not indented properly?
    // JASNOTE: Refactored by VS2022-cut/paste.

    struct source_file* first_file;
    struct source_line* current_line;
    // JASNOTE: I know I'm an old dog who can't learn new tricks,
    // but declaring a 1K local variable seems to just be asking for trouble?
    ...
    if (
        ...
        if (!strcmp(tab_element[0], "\\")) {
            ...
            if (strlen(buffer_error) > 0 && nb_byte == 0xFFFF) {
                ...
                mem_free_table(nb_element, tab_element);
                // JASNOTE: Is there any safe way to use sprintf?
                // Clang correctly points out that this may be a buffer overflow, depending on the lenth of the strings,
                // but I'm having a hard time thinking through how to make this "safe", orther than "don't".
                // n.b. I believe this is true of all of the uses of sprintf in this file / codebase.
                sprintf(
                    buffer_error_rtn,
...
static void BuildOneDataLineOperand(struct source_line* current_line, char* buffer_error_rtn, struct omf_segment* current_omfsegment) {
    ...
    if (
        ...
        if (nb_element == 3) {
            ...
            one_dword = (DWORD)EvalExpressionAsInteger(
                tab_element[2],
                buffer_error,
                current_line,
                1,
