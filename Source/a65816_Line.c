    ...
    for (current_line = first_line; current_line; current_line = current_line->next) {
        ...
        if (current_line->type == LINE_UNKNOWN) {
            if (current_macro != NULL) {
                printf(
                    "      => [Error] Unknown Macro line '%s' from Macro file '%s' (line %d), inserted in source file '%s' (line %d).\n",
                    current_line->line_data,
                    current_macro->file_name,
                    current_macro->file_line_number,
                    current_line->file->file_name,
                    current_line->file_line_number
                );
            } else {
                printf(
                    "      => [Error] Unknown line '%s' in source file '%s' (line %d).\n",
                    current_line->line_data,
                    current_line->file->file_name,
                    current_line->file_line_number
                );
            ...
            for (found = 0, current_line = first_file->first_line; current_line; current_line = current_line->next) {
                if (!strcmp(current_line->label_txt, current_global->name) && current_line->type == LINE_GLOBAL) {
                    ...
                    if (new_line == NULL) {
                        sprintf(
                            param->buffer_error,
                            "Impossible to allocate memory to Duplicate Line %d in Source file '%s'",
                            current_line->file_line_number,
                            current_line->file->file_path
                        );
            ...
            if (found == 0) {
                ...
                if (new_line == NULL) {
                    sprintf(
                        param->buffer_error,
                        "Impossible to allocate memory to Duplicate Line %d in Source file '%s'",
                        current_global->source_line->file_line_number,
                        current_global->source_line->file->file_path
                    );
                    my_RaiseError(ERROR_RAISE, param->buffer_error);
                    return(1);
                }
    ...
    if (current_macro == NULL) {
        ...
        for (current_line = first_line; current_line; current_line = current_line->next) {
            ...
            if (current_line->type == LINE_DIRECTIVE && !my_stricmp(current_line->opcode_txt, "ELSE")) {
                ...
                if (do_level == 0) {
                    printf(
                        "      => [Error] ELSE directive found in source file '%s' at line %d doesn't match with a previous (missing) DO or IF.\n",
                        current_line->file->file_name,
                        current_line->file_line_number
                    );
                    return(1);
            ...
            if (current_line->type == LINE_DIRECTIVE && !my_stricmp(current_line->opcode_txt, "FIN")) {
                ...
                if (do_level < 0) {
                    printf(
                        "      => [Error] FIN directive found in source file '%s' at line %d doesn't match with a previous (missing) DO or IF.\n",
                        current_line->file->file_name,
                        current_line->file_line_number
                    );
                    return(1);
        ...
        if (do_level > 0) {
            printf(
                "      => [Error] Missing FIN directive in source file '%s' (due to previous usage of a DO or IF directive).\n",
...
int ProcessMacroLineVariableLabel(struct macro_line* first_line, struct macro* current_macro) {
    ...
    for (begin_variable_line = first_line; begin_variable_line; begin_variable_line = begin_variable_line->next) {
        ...
        if (!strcmp(begin_variable_line->opcode, "=")) {
            ...
            if (strlen(buffer_error) > 0) {
                printf(
                    "    Error : Impossible to analyze Operand '%s' in Macro %s : %s.\n",
                    begin_variable_line->operand,
                    current_macro->name,
                    buffer_error
                );
                return(1);
        ...
        if (begin_variable_line->label[0] == ']') {
            ...
            for (replace_line = begin_variable_line; replace_line != end_variable_line; replace_line = replace_line->next) {
                ...
                new_operand = ReplaceInOperand(
                    replace_line->operand,
                    begin_variable_line->label,
                    unique_label,
...
int EvaluateVariableLine(struct source_line* current_line, struct omf_segment* current_omfsegment) {
    ...
    my_Memory(MEMORY_SEARCH_VARIABLE, current_line->label_txt, &current_variable, current_omfsegment);
    if (current_variable == NULL) {
        // JASNOTE: Changed "bad" to 1, 'cause 0 is "OK"?
        return(1);
    }
    ...
    if (strlen(buffer_error) > 0) {
        sprintf(
            param->buffer_error,
            "Impossible to evaluate Variable '%s' value '%s' (line %d, file '%s') : %s",
...
static void AddDateLine(struct source_line* current_line) {
    ...
    if (new_line == NULL) {
        printf(
            "      => [Error] Impossible to allocate memory to insert DAT value (line %d, file '%s')\n",
            current_line->file_line_number,
            current_line->file->file_path
        );
        return;
