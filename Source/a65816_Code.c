...
static void BuildOneCodeLineOperand(
    ...
    if ((current_line->opcode_byte == 0x54 /*MVN*/ || current_line->opcode_byte == 0x44 /*MVP*/) && next_sep != NULL) {
        ...
        *next_sep = '\0';
        ...
        /* Expression 1 */
        operand_value_1 = EvalExpressionAsInteger(
        ...
        if (strlen(buffer_error) > 0) {
            sprintf(
                param->buffer_error,
                "Impossible to evaluate Operand '%s' for instruction '%s  %s' (line %d, file '%s') : %s",
                param->buffer_operand,
                current_line->opcode_txt,
                current_line->operand_txt,
                current_line->file_line_number,
        ...
        /* Expression 2 */
        operand_value_2 = EvalExpressionAsInteger(
            next_sep + 1,
            buffer_error,
            current_line,
            1,
        ...
        operand_value_64 = EvalExpressionAsInteger(
            param->buffer_operand,
            buffer_error,
            current_line,
            operand_size,
        ...
        if (current_line->operand_txt[0] == '^') {
            /* @TODO: EvalExpressionAsInteger should always return bank for ^ usage,
             * regardless of operand or mode! When fixed, remove this if code! */
    ...
    if (current_line->address_mode == AM_PC_RELATIVE) {
        ...
        if ((current_line->address + 2) > operand_value) {
            ...
            if (delta > 128) {
                ...
                if (*has_error_rtn == 0) {
                    sprintf(
                        param->buffer_latest_error,
                        "Bad Jump for instruction '%s  %s' (line %d, file '%s') : Too long",
                        current_line->opcode_txt,
                        current_line->operand_txt,
                        current_line->file_line_number,
                        current_line->file->file_name
    ...
    if ((is_reloc_1 == 1 || is_reloc_2 == 1) &&
        ...
        if (is_reloc_1 == 1 && is_reloc_2 == 0) {
            ...
            /* Both are relocatable => we take the first */
            sprintf(current_line->reloc, "%c %d", (current_external_1 == NULL) ? ' ' : 'E', ByteCnt_1);
            if (BitShiftCnt_1 == 0xF0) {
                strcat(current_line->reloc, " >>16 ");
            } else if (BitShiftCnt_1 == 0xF8) {
...
static int GetOperandNbByte(
    ...
    if (is_address == 0) {
        ...
        if (has_more == 1) {
            ...
            /*@TODO: this may be a bug as it allows !, which should be "NOT" and not LONG...?) */
            /* | or ! */
            current_line->no_direct_page = 1;
            nb_max_byte = 3;
        } else if (has_less == 1) {
        ...
        if (nb_max_byte == 3 && is_block_copy == 0 && has_long_addr == 0 && has_more == 0) {
            /* Long Addressing is valid only if you have set Gold (or for MVP / MVN) */
            nb_max_byte = 2;
        }
