    ...
    for (begin_line = first_file->first_line; begin_line; begin_line = begin_line->next) {
        ...
        if (begin_line->type == LINE_VARIABLE) {
            ...
            if (current_variable == NULL) {
                ...
                sprintf(
                    param->buffer_error,
                    "Impossible to locate Variable '%s' (line %d from file '%s')",
                    begin_line->label_txt,
                    begin_line->file_line_number,
            ...
            value = EvalExpressionAsInteger(
                begin_line->operand_txt,
                buffer_error, begin_line,
                begin_line->nb_byte - 1,
                &is_reloc,
                &byte_count,
                &bit_shift,
                &offset_reference,
            ...
            if (strlen(buffer_error) > 0) {
                ...
                sprintf(
                    param->buffer_error,
                    "Impossible to evaluate Variable '%s' value '%s' (line %d from file '%s')",
                    begin_line->label_txt,
                    begin_line->operand_txt,
                    begin_line->file_line_number,
                    begin_line->file->file_name
        ...
        if (begin_line->type == LINE_DIRECTIVE && begin_line->is_inside_macro == 0 && !my_stricmp(begin_line->opcode_txt, "LUP")) {
            ...
            for (end_line = begin_line->next; end_line; end_line = end_line->next) {
                ...
                if (end_line->type == LINE_DIRECTIVE && !my_stricmp(end_line->opcode_txt, "--^")) {
                    ...
                    sprintf(
                        param->buffer_error,
                        "Impossible to locate end of Lup '--^', line %d from file '%s'",
                        begin_line->file_line_number,
                        begin_line->file->file_name
                    );
                    my_RaiseError(ERROR_RAISE, param->buffer_error);
...
static struct source_line* BuildLupLine(struct source_line* begin_line, struct source_line* end_line, struct omf_segment* current_omfsegment) {
    ...
    if (strlen(begin_line->operand_txt) == 0) {
        /* We have a PB because we have a LUP but without Nb,
         we will therefore consider that we LOOP 0 times (happens in the case of Macro with variable parameters) */
        nb_iter_64 = 0;
    } else {
        nb_iter_64 = EvalExpressionAsInteger(
            begin_line->operand_txt,
            buffer_error,
            begin_line,
            begin_line->nb_byte - 1,
        ...
        if (strlen(buffer_error) > 0) {
            sprintf(
                param->buffer_error,
                "Impossible to get Lup value '%s' (line %d from file '%s') : %s",
                begin_line->operand_txt,
                begin_line->file_line_number,
                begin_line->file->file_name,
                buffer_error
    ...
    if (nb_iter_64 <= 0 || nb_iter_64 > 0x8000) {
        ...
        if (strlen(begin_line->label_txt) > 0) {
            ...
            if (label_line == NULL) {
                sprintf(
                    param->buffer_error,
                    "Impossible to allocate memory to process line (line %d from file '%s')",
                    begin_line->file_line_number,
                    begin_line->file->file_name
                );
                my_RaiseError(ERROR_RAISE, param->buffer_error);
    ...
    for (current_line = begin_line->next; current_line != end_line; current_line = current_line->next) {
        ...
        /** Variable **/
            ...
            if (current_variable == NULL) {
                sprintf(
                    param->buffer_error,
                    "Impossible to find Variable '%s' declaration (line %d from file '%s') : %s",
            ...
            if (strlen(buffer_error) > 0) {
                sprintf(
                    param->buffer_error,
                    "Impossible to evaluate Variable '%s' value '%s' (line %d from file '%s') : %s",
                    current_line->label_txt,
                    current_line->operand_txt,
                    current_line->file_line_number,
                    current_line->file->file_name, buffer_error
                );
                my_RaiseError(ERROR_RAISE, param->buffer_error);
            }
    ...
    for (current_line = first_line; current_line; current_line = current_line->next) {
        ...
        if (strlen(current_line->label_txt) > 0 && current_line->label_txt[0] != ':' && current_line->label_txt[0] != ']') {
            ...
            if (current_line->label_txt == NULL) {
                sprintf(
                    param->buffer_error,
                    "Impossible to allocate memory to replace Lup at line %d from file '%s' [Update Label]",
                    begin_line->file_line_number,
                    begin_line->file->file_name
                );
                my_RaiseError(ERROR_RAISE, param->buffer_error);
...
static struct source_line* BuildSourceLupLine(struct source_line* current_source_line, int iter, struct omf_segment* current_omfsegment) {
    ...
    if (new_source_line->label_txt == NULL || new_source_line->opcode_txt == NULL || new_source_line->comment_txt == NULL) {
        ...
        sprintf(
            param->buffer_error,
            "Impossible to allocate memory to process Lup at line %d from file '%s'",
    ...
    for (i = 0; i < (int)strlen(new_source_line->label_txt); i++)
        if (new_source_line->label_txt[i] == '@') {
            if (iter > 26) {
                ...
                sprintf(
                    param->buffer_error,
                    "Impossible to update Label '%s' (iter=%d > 26) in Lup at line %d from file '%s'",
                    current_source_line->label_txt,
                    iter + 1,
                    current_source_line->file_line_number,
                    current_source_line->file->file_name
                );
    ...
    for (i = 0, l = 0; i < (int)strlen(current_source_line->operand_txt); i++) {
        if (current_source_line->operand_txt[i] == ']') {
            ...
            if (current_variable == NULL) {
                ...
                sprintf(
                    param->buffer_error,
                    "Impossible to find Lup Variable '%s' declaration (line %d from file '%s')",
                    current_source_line->label_txt,
                    current_source_line->file_line_number,
                    current_source_line->file->file_name
                );
